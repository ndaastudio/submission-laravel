			...
			<div class="me-7 mb-4">
				<div class="symbol symbol-100px symbol-lg-160px symbol-fixed position-relative">
					<div class="image-input image-input-empty">
						<img id="imgPreview" class="image-input-wrapper w-125px h-125px"
							src="{{ Auth::user()->foto ? asset('storage/' . Auth::user()->foto) : asset('media/avatars/blank.png') }}">
						<label class="btn btn-icon btn-circle btn-color-muted btn-active-color-primary w-25px h-25px bg-body shadow"
							...
							<input id="inputImage" type="file" name="foto" accept=".png, .jpg, .jpeg" />
						</label>
						<div class="fv-plugins-message-container invalid-feedback">{{ $errors->first('foto') }}</div>
					</div>
				</div>
			</div>
				...
				<div class="row">
					<div class="col-md-6">
						<input type="text" placeholder="Nama" name="nama" autocomplete="off"
							class="form-control form-control-solid {{ $errors->has('nama') ? 'is-invalid' : '' }}"
					...
					<div class="col-md-12 mt-4">
						<textarea type="text" class="form-control form-control-solid {{ $errors->has('bio') ? 'is-invalid' : '' }}"
						 placeholder="Bio" rows="5" name="bio">{{ old('bio', Auth::user()->bio) }}</textarea>
						@if ($errors->has('bio'))
							<div class="fv-plugins-message-container invalid-feedback">{{ $errors->first('bio') }}</div>
						@endif
					</div>
				</div>

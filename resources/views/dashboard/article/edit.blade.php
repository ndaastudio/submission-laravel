...
@section('content')
	<form action="{{ route('article.id', ['id' => $article->id]) }}" method="POST" enctype="multipart/form-data">
		@csrf
		@method('PUT')
		<div class="row">
			<div class="col-md-6">
				<input type="text" placeholder="Judul" name="judul" autocomplete="off"
					class="form-control form-control-solid {{ $errors->has('judul') ? 'is-invalid' : '' }}"
			...
			<div class="col-md-12 mt-4">
				<textarea type="text"
				 ...
				 placeholder="Deskripsi Singkat" rows="5" name="deskripsi_singkat">{{ old('deskripsi_singkat', $article->deskripsi_singkat) }}</textarea>
